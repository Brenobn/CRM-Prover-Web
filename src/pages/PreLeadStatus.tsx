...
import { PreLeadStatusForm } from '../components/PreLeadStatusForm'
import { ArrowUpDown, FilePen, Trash2 } from 'lucide-react'
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from '../components/ui/alert-dialog'
import { data as initialData, type StatusPreLead } from './PreLeadStatusColumns'

export function PreLeadStatus() {
  const [data, setData] = useState<StatusPreLead[]>(initialData)
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])
  const [isSheetOpen, setIsSheetOpen] = useState(false)
  const [editingStatus, setEditingStatus] = useState<StatusPreLead | null>(null)
  const [deletingStatus, setDeletingStatus] = useState<StatusPreLead | null>(null)
  const [sorting, setSorting] = useState<SortingState>([])

  const columns = useMemo<ColumnDef<StatusPreLead>[]>(
    () => [
    ...
    <div className="space-y-4">
      <header className="flex items-start justify-between">
        <div className="space-y-1.5">
          <h1 className="text-2xl font-normal text-gray-150 dark:text-gray-75 -tracking-tight">Status Pré Lead</h1>
          <div className="text-sm text-muted-foreground">
            <a href="#" className="text-blue-500 hover:underline">Home</a>
            <span> / Status Pré Lead</span>
          </div>
        </div>
        <Sheet open={isSheetOpen} onOpenChange={(isOpen) => { setIsSheetOpen(isOpen); if (!isOpen) { setEditingStatus(null) } }}>
          ...
          <SheetContent className="flex flex-col sm:max-w-md p-0 shadow-xl shadow-gray-125">
            <SheetHeader className="text-left p-6 pb-4 border-b">
              <SheetTitle>{editingStatus ? "Editar Status" : "Adicionar Novo Status"}</SheetTitle>
              <SheetDescription>{editingStatus ? "Altere os dados abaixo." : "Preencha os dados para cadastrar."}</SheetDescription>
            </SheetHeader>
            <div className="flex-1 overflow-auto p-6">
              <PreLeadStatusForm onSubmit={handleFormSubmit} initialData={editingStatus ?? undefined} />
            </div>
          </SheetContent>
        </Sheet>
      </header>
