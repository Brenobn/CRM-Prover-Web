...
import { SaleConclusionReasonForm } from '../components/SaleConclusionReasonForm'
import { ArrowUpDown, FilePen, Trash2 } from 'lucide-react'
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from '../components/ui/alert-dialog'
import { data as initialData, type MotivoConclusãoVenda } from './SaleConclusionReasonColumn'

export function SaleConclusionReason() {
  const [data, setData] = useState<MotivoConclusãoVenda[]>(initialData)
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])
  const [isSheetOpen, setIsSheetOpen] = useState(false)
  const [editingReason, setEditingReason] = useState<MotivoConclusãoVenda | null>(null)
  const [deletingReason, setDeletingReason] = useState<MotivoConclusãoVenda | null>(null)
  const [sorting, setSorting] = useState<SortingState>([])

  const columns = useMemo<ColumnDef<MotivoConclusãoVenda>[]>(
    () => [
  ...
  function handleFormSubmit(values: { descrição: string; ativo: boolean }) {
    if (editingReason) {
      ...
      const newReason: MotivoConclusãoVenda = {
    ...
    <div className="space-y-4">
      <header className="flex items-start justify-between">
        <div className="space-y-1.5">
          <h1 className="text-2xl font-normal text-gray-150 dark:text-gray-75 -tracking-tight">Motivo Conclusão Venda</h1>
          <div className="text-sm text-muted-foreground">
            <a href="#" className="text-blue-500 hover:underline">Home</a>
            <span> / Motivo Conclusão Venda</span>
          </div>
        </div>
        <Sheet open={isSheetOpen} onOpenChange={(isOpen) => { setIsSheetOpen(isOpen); if (!isOpen) { setEditingReason(null) } }}>
          ...
          <SheetContent className="flex flex-col sm:max-w-md p-0 shadow-xl shadow-gray-125">
            ...
            <div className="flex-1 overflow-auto p-6">
              <SaleConclusionReasonForm onSubmit={handleFormSubmit} initialData={editingReason ?? undefined} />
            </div>
          </SheetContent>
        </Sheet>
      </header>
